...
def get_all_centredecongres(detailed: bool = False, database: Session = Depends(get_db)) -> list:
    ...
    if detailed:
        ...
        for centredecongres_item in centredecongres_list:
            ...
            gestionnaire_list = database.query(Gestionnaire).filter(Gestionnaire.centredecongres_1_id == centredecongres_item.id).all()
...
def get_paginated_centredecongres(skip: int = 0, limit: int = 100, detailed: bool = False, database: Session = Depends(get_db)) -> dict:
    ...
    result = []
    for centredecongres_item in centredecongres_list:
        gestionnaire_1_ids = database.query(Gestionnaire.id).filter(Gestionnaire.centredecongres_1_id == centredecongres_item.id).all()
        ...
        item_data = {
            ...
            "gestionnaire_1_ids": [x[0] for x in gestionnaire_1_ids],            "evenementsalle_1_ids": [x[0] for x in evenementsalle_1_ids]        }
...
async def get_centredecongres(centredecongres_id: int, database: Session = Depends(get_db)) -> CentredeCongres:
    ...
    gestionnaire_1_ids = database.query(Gestionnaire.id).filter(Gestionnaire.centredecongres_1_id == db_centredecongres.id).all()
...
async def create_centredecongres(centredecongres_data: CentredeCongresCreate, database: Session = Depends(get_db)) -> CentredeCongres:
    ...
    db_centredecongres = CentredeCongres(
        adresse=centredecongres_data.adresse,        nom=centredecongres_data.nom        )

    database.add(db_centredecongres)
    database.commit()
    database.refresh(db_centredecongres)

    if centredecongres_data.gestionnaire_1:
        # Validate that all Gestionnaire IDs exist
        for gestionnaire_id in centredecongres_data.gestionnaire_1:
        ...
        database.query(Gestionnaire).filter(Gestionnaire.id.in_(centredecongres_data.gestionnaire_1)).update(
            {Gestionnaire.centredecongres_1_id: db_centredecongres.id}, synchronize_session=False
        )
        database.commit()
...
async def update_centredecongres(centredecongres_id: int, centredecongres_data: CentredeCongresCreate, database: Session = Depends(get_db)) -> CentredeCongres:
    ...
    if centredecongres_data.gestionnaire_1 is not None:
        ...
        database.query(Gestionnaire).filter(Gestionnaire.centredecongres_1_id == db_centredecongres.id).update(
            {Gestionnaire.centredecongres_1_id: None}, synchronize_session=False
        )
...
def get_all_tarifs(detailed: bool = False, database: Session = Depends(get_db)) -> list:
    ...
    if detailed:
        ...
        for tarifs_item in tarifs_list:
            ...
            reservation_list = database.query(Reservation).join(tarifs_reservation, Reservation.id == tarifs_reservation.c.reservation_4).filter(tarifs_reservation.c.tarifs_2 == tarifs_item.id).all()
...
def get_paginated_tarifs(skip: int = 0, limit: int = 100, detailed: bool = False, database: Session = Depends(get_db)) -> dict:
    ...
    for tarifs_item in tarifs_list:
        ...
        reservation_ids = database.query(tarifs_reservation.c.reservation_4).filter(tarifs_reservation.c.tarifs_2 == tarifs_item.id).all()
...
async def get_tarifs(tarifs_id: int, database: Session = Depends(get_db)) -> Tarifs:
    ...
    reservation_ids = database.query(tarifs_reservation.c.reservation_4).filter(tarifs_reservation.c.tarifs_2 == db_tarifs.id).all()
...
async def create_tarifs(tarifs_data: TarifsCreate, database: Session = Depends(get_db)) -> Tarifs:
    ...
    if tarifs_data.reservation_4:
        for id in tarifs_data.reservation_4:
            # Entity already validated before creation
    ...
    if tarifs_data.reservation_4:
        for id in tarifs_data.reservation_4:
            ...
            db_reservation = database.query(Reservation).filter(Reservation.id == id).first()
            # Create the association
            association = tarifs_reservation.insert().values(tarifs_2=db_tarifs.id, reservation_4=db_reservation.id)
            database.execute(association)
            database.commit()
    if tarifs_data.evenementsalle_2:
        for id in tarifs_data.evenementsalle_2:
...
async def update_tarifs(tarifs_id: int, tarifs_data: TarifsCreate, database: Session = Depends(get_db)) -> Tarifs:
    ...
    for materielsprestations_id in new_materielsprestations_ids:
        ...
        database.execute(association)
    existing_reservation_ids = [assoc.reservation_4 for assoc in database.execute(
        tarifs_reservation.select().where(tarifs_reservation.c.tarifs_2 == db_tarifs.id))]

    reservations_to_remove = set(existing_reservation_ids) - set(tarifs_data.reservation_4)
    for reservation_id in reservations_to_remove:
        association = tarifs_reservation.delete().where(
            (tarifs_reservation.c.tarifs_2 == db_tarifs.id) & (tarifs_reservation.c.reservation_4 == reservation_id))
        database.execute(association)

    new_reservation_ids = set(tarifs_data.reservation_4) - set(existing_reservation_ids)
    for reservation_id in new_reservation_ids:
        ...
        association = tarifs_reservation.insert().values(reservation_4=db_reservation.id, tarifs_2=db_tarifs.id)
        database.execute(association)
...
@app.post("/tarifs/{tarifs_id}/reservation_4/{reservation_id}/", response_model=None, tags=["Tarifs Relationships"])
async def add_reservation_4_to_tarifs(tarifs_id: int, reservation_id: int, database: Session = Depends(get_db)):
    """Add a Reservation to this Tarifs's reservation_4 relationship"""
    db_tarifs = database.query(Tarifs).filter(Tarifs.id == tarifs_id).first()
    if db_tarifs is None:
        raise HTTPException(status_code=404, detail="Tarifs not found")
    ...
    # Check if relationship already exists
    existing = database.query(tarifs_reservation).filter(
        (tarifs_reservation.c.tarifs_2 == tarifs_id) &
        (tarifs_reservation.c.reservation_4 == reservation_id)
    ).first()
    ...
    # Create the association
    association = tarifs_reservation.insert().values(tarifs_2=tarifs_id, reservation_4=reservation_id)
    database.execute(association)
    database.commit()

    return {"message": "Reservation added to reservation_4 successfully"}


@app.delete("/tarifs/{tarifs_id}/reservation_4/{reservation_id}/", response_model=None, tags=["Tarifs Relationships"])
async def remove_reservation_4_from_tarifs(tarifs_id: int, reservation_id: int, database: Session = Depends(get_db)):
    """Remove a Reservation from this Tarifs's reservation_4 relationship"""
    db_tarifs = database.query(Tarifs).filter(Tarifs.id == tarifs_id).first()
    if db_tarifs is None:
        raise HTTPException(status_code=404, detail="Tarifs not found")
    ...
    return {"message": "Reservation removed from reservation_4 successfully"}


@app.get("/tarifs/{tarifs_id}/reservation_4/", response_model=None, tags=["Tarifs Relationships"])
async def get_reservation_4_of_tarifs(tarifs_id: int, database: Session = Depends(get_db)):
    """Get all Reservation entities related to this Tarifs through reservation_4"""
    db_tarifs = database.query(Tarifs).filter(Tarifs.id == tarifs_id).first()
    if db_tarifs is None:
        raise HTTPException(status_code=404, detail="Tarifs not found")

    reservation_ids = database.query(tarifs_reservation.c.reservation_4).filter(tarifs_reservation.c.tarifs_2 == tarifs_id).all()
    reservation_list = database.query(Reservation).filter(Reservation.id.in_([id[0] for id in reservation_ids])).all()

    return {
        "tarifs_id": tarifs_id,
        "reservation_4_count": len(reservation_list),
        "reservation_4": reservation_list
    }
...
async def create_evenementsalle(evenementsalle_data: EvenementSalleCreate, database: Session = Depends(get_db)) -> EvenementSalle:
    ...
    db_evenementsalle = EvenementSalle(
        nom=evenementsalle_data.nom,        capaciteMax=evenementsalle_data.capaciteMax,        typeElement=evenementsalle_data.typeElement,        reservation_id=evenementsalle_data.reservation,        centredecongres_id=evenementsalle_data.centredecongres        )
...
async def bulk_create_evenementsalle(items: list[EvenementSalleCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_evenementsalle = EvenementSalle(
                nom=item_data.nom,                capaciteMax=item_data.capaciteMax,                typeElement=item_data.typeElement,                reservation_id=item_data.reservation,                centredecongres_id=item_data.centredecongres            )
            database.add(db_evenementsalle)
            database.flush()  # Get ID without committing
...
@app.post("/evenementsalle/methods/verifierCapacite/", response_model=None, tags=["EvenementSalle Methods"])
async def evenementsalle_verifierCapacite(
    params: dict = Body(default=None, embed=True),
    database: Session = Depends(get_db)
):
    """
    Execute the verifierCapacite class method on EvenementSalle.
    This method operates on all EvenementSalle entities or performs class-level operations.
...
@app.post("/evenementsalle/methods/estDisponible/", response_model=None, tags=["EvenementSalle Methods"])
async def evenementsalle_estDisponible(
    params: dict = Body(default=None, embed=True),
    database: Session = Depends(get_db)
):
    """
    Execute the estDisponible class method on EvenementSalle.
    This method operates on all EvenementSalle entities or performs class-level operations.
...
def get_all_materielsprestations(detailed: bool = False, database: Session = Depends(get_db)) -> list:
    ...
    if detailed:
        ...
        for materielsprestations_item in materielsprestations_list:
            ...
            # Add many-to-many and one-to-many relationship objects (full details)
            reservation_list = database.query(Reservation).join(inclut, Reservation.id == inclut.c.reservation_3).filter(inclut.c.materielsprestations == materielsprestations_item.id).all()
...
def get_paginated_materielsprestations(skip: int = 0, limit: int = 100, detailed: bool = False, database: Session = Depends(get_db)) -> dict:
    ...
    result = []
    for materielsprestations_item in materielsprestations_list:
        reservation_ids = database.query(inclut.c.reservation_3).filter(inclut.c.materielsprestations == materielsprestations_item.id).all()
...
async def get_materielsprestations(materielsprestations_id: int, database: Session = Depends(get_db)) -> Materielsprestations:
    ...
    reservation_ids = database.query(inclut.c.reservation_3).filter(inclut.c.materielsprestations == db_materielsprestations.id).all()
...
async def create_materielsprestations(materielsprestations_data: MaterielsprestationsCreate, database: Session = Depends(get_db)) -> Materielsprestations:
    ...
    if not materielsprestations_data.reservation_3 or len(materielsprestations_data.reservation_3) < 1:
        raise HTTPException(status_code=400, detail="At least 1 Reservation(s) required")
    if materielsprestations_data.reservation_3:
        for id in materielsprestations_data.reservation_3:
            # Entity already validated before creation
            db_reservation = database.query(Reservation).filter(Reservation.id == id).first()
            if not db_reservation:
                raise HTTPException(status_code=404, detail=f"Reservation with ID {id} not found")
    ...
    db_materielsprestations = Materielsprestations(
        quantiteMax=materielsprestations_data.quantiteMax,        type=materielsprestations_data.type,        nom=materielsprestations_data.nom,        prixUnitaire=materielsprestations_data.prixUnitaire        )
    ...
    if materielsprestations_data.reservation_3:
        for id in materielsprestations_data.reservation_3:
            ...
            db_reservation = database.query(Reservation).filter(Reservation.id == id).first()
            # Create the association
            association = inclut.insert().values(materielsprestations=db_materielsprestations.id, reservation_3=db_reservation.id)
            database.execute(association)
            database.commit()
    if materielsprestations_data.tarifs_1:
        for id in materielsprestations_data.tarifs_1:
...
async def bulk_create_materielsprestations(items: list[MaterielsprestationsCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_materielsprestations = Materielsprestations(
                quantiteMax=item_data.quantiteMax,                type=item_data.type,                nom=item_data.nom,                prixUnitaire=item_data.prixUnitaire            )
            database.add(db_materielsprestations)
            database.flush()  # Get ID without committing
...
async def update_materielsprestations(materielsprestations_id: int, materielsprestations_data: MaterielsprestationsCreate, database: Session = Depends(get_db)) -> Materielsprestations:
    ...
    existing_reservation_ids = [assoc.reservation_3 for assoc in database.execute(
        inclut.select().where(inclut.c.materielsprestations == db_materielsprestations.id))]

    reservations_to_remove = set(existing_reservation_ids) - set(materielsprestations_data.reservation_3)
    for reservation_id in reservations_to_remove:
        association = inclut.delete().where(
            (inclut.c.materielsprestations == db_materielsprestations.id) & (inclut.c.reservation_3 == reservation_id))
        database.execute(association)

    new_reservation_ids = set(materielsprestations_data.reservation_3) - set(existing_reservation_ids)
    for reservation_id in new_reservation_ids:
        db_reservation = database.query(Reservation).filter(Reservation.id == reservation_id).first()
        if db_reservation is None:
            raise HTTPException(status_code=404, detail=f"Reservation with ID {reservation_id} not found")
        association = inclut.insert().values(reservation_3=db_reservation.id, materielsprestations=db_materielsprestations.id)
        database.execute(association)
...
@app.post("/materielsprestations/{materielsprestations_id}/reservation_3/{reservation_id}/", response_model=None, tags=["Materielsprestations Relationships"])
async def add_reservation_3_to_materielsprestations(materielsprestations_id: int, reservation_id: int, database: Session = Depends(get_db)):
    """Add a Reservation to this Materielsprestations's reservation_3 relationship"""
    db_materielsprestations = database.query(Materielsprestations).filter(Materielsprestations.id == materielsprestations_id).first()
    if db_materielsprestations is None:
        raise HTTPException(status_code=404, detail="Materielsprestations not found")
    ...
    # Check if relationship already exists
    existing = database.query(inclut).filter(
        (inclut.c.materielsprestations == materielsprestations_id) &
        (inclut.c.reservation_3 == reservation_id)
    ).first()
    ...
    # Create the association
    association = inclut.insert().values(materielsprestations=materielsprestations_id, reservation_3=reservation_id)
    database.execute(association)
    database.commit()

    return {"message": "Reservation added to reservation_3 successfully"}


@app.delete("/materielsprestations/{materielsprestations_id}/reservation_3/{reservation_id}/", response_model=None, tags=["Materielsprestations Relationships"])
async def remove_reservation_3_from_materielsprestations(materielsprestations_id: int, reservation_id: int, database: Session = Depends(get_db)):
    """Remove a Reservation from this Materielsprestations's reservation_3 relationship"""
    db_materielsprestations = database.query(Materielsprestations).filter(Materielsprestations.id == materielsprestations_id).first()
    if db_materielsprestations is None:
        raise HTTPException(status_code=404, detail="Materielsprestations not found")
    ...
    return {"message": "Reservation removed from reservation_3 successfully"}


@app.get("/materielsprestations/{materielsprestations_id}/reservation_3/", response_model=None, tags=["Materielsprestations Relationships"])
async def get_reservation_3_of_materielsprestations(materielsprestations_id: int, database: Session = Depends(get_db)):
    """Get all Reservation entities related to this Materielsprestations through reservation_3"""
    db_materielsprestations = database.query(Materielsprestations).filter(Materielsprestations.id == materielsprestations_id).first()
    if db_materielsprestations is None:
        raise HTTPException(status_code=404, detail="Materielsprestations not found")

    reservation_ids = database.query(inclut.c.reservation_3).filter(inclut.c.materielsprestations == materielsprestations_id).all()
    reservation_list = database.query(Reservation).filter(Reservation.id.in_([id[0] for id in reservation_ids])).all()

    return {
        "materielsprestations_id": materielsprestations_id,
        "reservation_3_count": len(reservation_list),
        "reservation_3": reservation_list
    }
...
@app.post("/materielsprestations/methods/verifierStockDisponible/", response_model=None, tags=["Materielsprestations Methods"])
async def materielsprestations_verifierStockDisponible(
    params: dict = Body(default=None, embed=True),
    database: Session = Depends(get_db)
):
    """
    Execute the verifierStockDisponible class method on Materielsprestations.
    This method operates on all Materielsprestations entities or performs class-level operations.

    Parameters (pass as JSON body):
    - quantiteDemandee: Any    """
    try:
        # Capture stdout to include print outputs in the response
        import io
        import sys
...
async def create_disponibilites(disponibilites_data: DisponibilitesCreate, database: Session = Depends(get_db)) -> Disponibilites:
    ...
    db_disponibilites = Disponibilites(
        dateDebut=disponibilites_data.dateDebut,        dureeMinim=disponibilites_data.dureeMinim,        dateFin=disponibilites_data.dateFin,        motifDisponibilite=disponibilites_data.motifDisponibilite        )
...
async def bulk_create_disponibilites(items: list[DisponibilitesCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_disponibilites = Disponibilites(
                dateDebut=item_data.dateDebut,                dureeMinim=item_data.dureeMinim,                dateFin=item_data.dateFin,                motifDisponibilite=item_data.motifDisponibilite            )
            database.add(db_disponibilites)
            database.flush()  # Get ID without committing
...
@app.post("/disponibilites/methods/ajouterIndisponibilite/", response_model=None, tags=["Disponibilites Methods"])
async def disponibilites_ajouterIndisponibilite(
    params: dict = Body(default=None, embed=True),
    database: Session = Depends(get_db)
):
    """
    Execute the ajouterIndisponibilite class method on Disponibilites.
    This method operates on all Disponibilites entities or performs class-level operations.

    Parameters (pass as JSON body):
    - motif: Any    - debut: Any    - fin: Any    """
    try:
        # Capture stdout to include print outputs in the response
        import io
        import sys
...
def get_all_reservation(detailed: bool = False, database: Session = Depends(get_db)) -> list:
    ...
    if detailed:
        ...
        for reservation_item in reservation_list:
            ...
            # Add many-to-many and one-to-many relationship objects (full details)
            materielsprestations_list = database.query(Materielsprestations).join(inclut, Materielsprestations.id == inclut.c.materielsprestations).filter(inclut.c.reservation_3 == reservation_item.id).all()
            item_dict['materielsprestations'] = []
            for materielsprestations_obj in materielsprestations_list:
                materielsprestations_dict = materielsprestations_obj.__dict__.copy()
                materielsprestations_dict.pop('_sa_instance_state', None)
                item_dict['materielsprestations'].append(materielsprestations_dict)
            tarifs_list = database.query(Tarifs).join(tarifs_reservation, Tarifs.id == tarifs_reservation.c.tarifs_2).filter(tarifs_reservation.c.reservation_4 == reservation_item.id).all()
            item_dict['tarifs_2'] = []
            for tarifs_obj in tarifs_list:
                tarifs_dict = tarifs_obj.__dict__.copy()
                tarifs_dict.pop('_sa_instance_state', None)
                item_dict['tarifs_2'].append(tarifs_dict)
...
def get_paginated_reservation(skip: int = 0, limit: int = 100, detailed: bool = False, database: Session = Depends(get_db)) -> dict:
    ...
    result = []
    for reservation_item in reservation_list:
        materielsprestations_ids = database.query(inclut.c.materielsprestations).filter(inclut.c.reservation_3 == reservation_item.id).all()
        tarifs_ids = database.query(tarifs_reservation.c.tarifs_2).filter(tarifs_reservation.c.reservation_4 == reservation_item.id).all()
        ...
        item_data = {
            ...
            "gestionnaire_ids": [x[0] for x in gestionnaire_ids],            "evenementsalle_ids": [x[0] for x in evenementsalle_ids]        }
        result.append(item_data)
    return {
        "total": total,
        "skip": skip,
...
async def get_reservation(reservation_id: int, database: Session = Depends(get_db)) -> Reservation:
    ...
    materielsprestations_ids = database.query(inclut.c.materielsprestations).filter(inclut.c.reservation_3 == db_reservation.id).all()
    tarifs_ids = database.query(tarifs_reservation.c.tarifs_2).filter(tarifs_reservation.c.reservation_4 == db_reservation.id).all()
...
async def create_reservation(reservation_data: ReservationCreate, database: Session = Depends(get_db)) -> Reservation:
    ...
    if reservation_data.materielsprestations:
        for id in reservation_data.materielsprestations:
            # Entity already validated before creation
            db_materielsprestations = database.query(Materielsprestations).filter(Materielsprestations.id == id).first()
            if not db_materielsprestations:
                raise HTTPException(status_code=404, detail=f"Materielsprestations with ID {id} not found")
    if reservation_data.tarifs_2:
        for id in reservation_data.tarifs_2:
            # Entity already validated before creation
            db_tarifs = database.query(Tarifs).filter(Tarifs.id == id).first()
            if not db_tarifs:
                raise HTTPException(status_code=404, detail=f"Tarifs with ID {id} not found")
    ...
    db_reservation = Reservation(
        dateDebut=reservation_data.dateDebut,        coutTotal=reservation_data.coutTotal,        nomEvenement=reservation_data.nomEvenement,        estConfirmee=reservation_data.estConfirmee,        nbParticipantsPrevu=reservation_data.nbParticipantsPrevu,        etatActuel=reservation_data.etatActuel,        emailReferent=reservation_data.emailReferent,        dateFin=reservation_data.dateFin,        description=reservation_data.description,        delaiConfirmation=reservation_data.delaiConfirmation        )
...
async def bulk_create_reservation(items: list[ReservationCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_reservation = Reservation(
                dateDebut=item_data.dateDebut,                coutTotal=item_data.coutTotal,                nomEvenement=item_data.nomEvenement,                estConfirmee=item_data.estConfirmee,                nbParticipantsPrevu=item_data.nbParticipantsPrevu,                etatActuel=item_data.etatActuel,                emailReferent=item_data.emailReferent,                dateFin=item_data.dateFin,                description=item_data.description,                delaiConfirmation=item_data.delaiConfirmation            )
            database.add(db_reservation)
            database.flush()  # Get ID without committing
...
async def update_reservation(reservation_id: int, reservation_data: ReservationCreate, database: Session = Depends(get_db)) -> Reservation:
    ...
    existing_materielsprestations_ids = [assoc.materielsprestations for assoc in database.execute(
        inclut.select().where(inclut.c.reservation_3 == db_reservation.id))]

    materielsprestationss_to_remove = set(existing_materielsprestations_ids) - set(reservation_data.materielsprestations)
    for materielsprestations_id in materielsprestationss_to_remove:
        association = inclut.delete().where(
            (inclut.c.reservation_3 == db_reservation.id) & (inclut.c.materielsprestations == materielsprestations_id))
        database.execute(association)

    new_materielsprestations_ids = set(reservation_data.materielsprestations) - set(existing_materielsprestations_ids)
    for materielsprestations_id in new_materielsprestations_ids:
        db_materielsprestations = database.query(Materielsprestations).filter(Materielsprestations.id == materielsprestations_id).first()
        if db_materielsprestations is None:
            raise HTTPException(status_code=404, detail=f"Materielsprestations with ID {materielsprestations_id} not found")
        association = inclut.insert().values(materielsprestations=db_materielsprestations.id, reservation_3=db_reservation.id)
        database.execute(association)
    existing_tarifs_ids = [assoc.tarifs_2 for assoc in database.execute(
        tarifs_reservation.select().where(tarifs_reservation.c.reservation_4 == db_reservation.id))]

    tarifss_to_remove = set(existing_tarifs_ids) - set(reservation_data.tarifs_2)
    for tarifs_id in tarifss_to_remove:
        association = tarifs_reservation.delete().where(
            (tarifs_reservation.c.reservation_4 == db_reservation.id) & (tarifs_reservation.c.tarifs_2 == tarifs_id))
        database.execute(association)

    new_tarifs_ids = set(reservation_data.tarifs_2) - set(existing_tarifs_ids)
    for tarifs_id in new_tarifs_ids:
        db_tarifs = database.query(Tarifs).filter(Tarifs.id == tarifs_id).first()
        if db_tarifs is None:
            raise HTTPException(status_code=404, detail=f"Tarifs with ID {tarifs_id} not found")
...
@app.post("/reservation/{reservation_id}/materielsprestations/{materielsprestations_id}/", response_model=None, tags=["Reservation Relationships"])
async def add_materielsprestations_to_reservation(reservation_id: int, materielsprestations_id: int, database: Session = Depends(get_db)):
    """Add a Materielsprestations to this Reservation's materielsprestations relationship"""
    db_reservation = database.query(Reservation).filter(Reservation.id == reservation_id).first()
    if db_reservation is None:
        raise HTTPException(status_code=404, detail="Reservation not found")
    ...
    # Create the association
    association = inclut.insert().values(reservation_3=reservation_id, materielsprestations=materielsprestations_id)
    database.execute(association)
    database.commit()

    return {"message": "Materielsprestations added to materielsprestations successfully"}


@app.delete("/reservation/{reservation_id}/materielsprestations/{materielsprestations_id}/", response_model=None, tags=["Reservation Relationships"])
async def remove_materielsprestations_from_reservation(reservation_id: int, materielsprestations_id: int, database: Session = Depends(get_db)):
    """Remove a Materielsprestations from this Reservation's materielsprestations relationship"""
    db_reservation = database.query(Reservation).filter(Reservation.id == reservation_id).first()
    if db_reservation is None:
        raise HTTPException(status_code=404, detail="Reservation not found")
    ...
    return {"message": "Materielsprestations removed from materielsprestations successfully"}


@app.get("/reservation/{reservation_id}/materielsprestations/", response_model=None, tags=["Reservation Relationships"])
async def get_materielsprestations_of_reservation(reservation_id: int, database: Session = Depends(get_db)):
    """Get all Materielsprestations entities related to this Reservation through materielsprestations"""
    db_reservation = database.query(Reservation).filter(Reservation.id == reservation_id).first()
    if db_reservation is None:
        raise HTTPException(status_code=404, detail="Reservation not found")

    materielsprestations_ids = database.query(inclut.c.materielsprestations).filter(inclut.c.reservation_3 == reservation_id).all()
    materielsprestations_list = database.query(Materielsprestations).filter(Materielsprestations.id.in_([id[0] for id in materielsprestations_ids])).all()

    return {
        "reservation_id": reservation_id,
        "materielsprestations_count": len(materielsprestations_list),
        "materielsprestations": materielsprestations_list
    }

@app.post("/reservation/{reservation_id}/tarifs_2/{tarifs_id}/", response_model=None, tags=["Reservation Relationships"])
async def add_tarifs_2_to_reservation(reservation_id: int, tarifs_id: int, database: Session = Depends(get_db)):
    """Add a Tarifs to this Reservation's tarifs_2 relationship"""
    db_reservation = database.query(Reservation).filter(Reservation.id == reservation_id).first()
    if db_reservation is None:
        raise HTTPException(status_code=404, detail="Reservation not found")
    ...
    # Create the association
    association = tarifs_reservation.insert().values(reservation_4=reservation_id, tarifs_2=tarifs_id)
    database.execute(association)
    database.commit()

    return {"message": "Tarifs added to tarifs_2 successfully"}


@app.delete("/reservation/{reservation_id}/tarifs_2/{tarifs_id}/", response_model=None, tags=["Reservation Relationships"])
async def remove_tarifs_2_from_reservation(reservation_id: int, tarifs_id: int, database: Session = Depends(get_db)):
    """Remove a Tarifs from this Reservation's tarifs_2 relationship"""
    db_reservation = database.query(Reservation).filter(Reservation.id == reservation_id).first()
    if db_reservation is None:
        raise HTTPException(status_code=404, detail="Reservation not found")
    ...
    return {"message": "Tarifs removed from tarifs_2 successfully"}


@app.get("/reservation/{reservation_id}/tarifs_2/", response_model=None, tags=["Reservation Relationships"])
async def get_tarifs_2_of_reservation(reservation_id: int, database: Session = Depends(get_db)):
    """Get all Tarifs entities related to this Reservation through tarifs_2"""
    db_reservation = database.query(Reservation).filter(Reservation.id == reservation_id).first()
    if db_reservation is None:
        raise HTTPException(status_code=404, detail="Reservation not found")

    tarifs_ids = database.query(tarifs_reservation.c.tarifs_2).filter(tarifs_reservation.c.reservation_4 == reservation_id).all()
    tarifs_list = database.query(Tarifs).filter(Tarifs.id.in_([id[0] for id in tarifs_ids])).all()

    return {
        "reservation_id": reservation_id,
        "tarifs_2_count": len(tarifs_list),
        "tarifs_2": tarifs_list
    }
...
@app.post("/reservation/methods/confirmerPaiement/", response_model=None, tags=["Reservation Methods"])
async def reservation_confirmerPaiement(
    database: Session = Depends(get_db)
):
    """
    Execute the confirmerPaiement class method on Reservation.
    This method operates on all Reservation entities or performs class-level operations.
    """
    try:
        # Capture stdout to include print outputs in the response
...
@app.post("/reservation/methods/calculerCout/", response_model=None, tags=["Reservation Methods"])
async def reservation_calculerCout(
    database: Session = Depends(get_db)
):
    """
    Execute the calculerCout class method on Reservation.
    This method operates on all Reservation entities or performs class-level operations.
    """
    try:
        # Capture stdout to include print outputs in the response
...
async def create_stats(stats_data: StatsCreate, database: Session = Depends(get_db)) -> Stats:
    ...
    db_stats = Stats(
        chiffresAffaires=stats_data.chiffresAffaires,        periode=stats_data.periode,        tauxOccupation=stats_data.tauxOccupation,        gestionnaire_2_id=stats_data.gestionnaire_2        )
...
async def bulk_create_stats(items: list[StatsCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_stats = Stats(
                chiffresAffaires=item_data.chiffresAffaires,                periode=item_data.periode,                tauxOccupation=item_data.tauxOccupation,                gestionnaire_2_id=item_data.gestionnaire_2            )
            database.add(db_stats)
            database.flush()  # Get ID without committing
...
@app.post("/stats/methods/calculerCA/", response_model=None, tags=["Stats Methods"])
async def stats_calculerCA(
    database: Session = Depends(get_db)
):
    """
    Execute the calculerCA class method on Stats.
    This method operates on all Stats entities or performs class-level operations.
    """
    try:
        # Capture stdout to include print outputs in the response
        import io
...
@app.post("/stats/methods/visualiserUtilisationRessources/", response_model=None, tags=["Stats Methods"])
async def stats_visualiserUtilisationRessources(
    database: Session = Depends(get_db)
):
    """
    Execute the visualiserUtilisationRessources class method on Stats.
    This method operates on all Stats entities or performs class-level operations.
    """
    try:
        # Capture stdout to include print outputs in the response
        ...
        return {
            "class": "Stats",
            "method": "visualiserUtilisationRessources",
            "status": "executed",
            "result": result,
            "output": output if output else None
        }
...
@app.post("/stats/methods/calculerTauxOccupation/", response_model=None, tags=["Stats Methods"])
async def stats_calculerTauxOccupation(
    database: Session = Depends(get_db)
):
    """
    Execute the calculerTauxOccupation class method on Stats.
    This method operates on all Stats entities or performs class-level operations.
    """
    try:
        # Capture stdout to include print outputs in the response
...
async def create_gestionnaire(gestionnaire_data: GestionnaireCreate, database: Session = Depends(get_db)) -> Gestionnaire:
    ...
    if gestionnaire_data.centredecongres_1 :
        db_centredecongres_1 = database.query(CentredeCongres).filter(CentredeCongres.id == gestionnaire_data.centredecongres_1).first()
    ...
    db_gestionnaire = Gestionnaire(
        identifiant=gestionnaire_data.identifiant,        nom=gestionnaire_data.nom,        centredecongres_1_id=gestionnaire_data.centredecongres_1,        reservation_1_id=gestionnaire_data.reservation_1        )
...
async def bulk_create_gestionnaire(items: list[GestionnaireCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_gestionnaire = Gestionnaire(
                identifiant=item_data.identifiant,                nom=item_data.nom,                centredecongres_1_id=item_data.centredecongres_1,                reservation_1_id=item_data.reservation_1            )
            database.add(db_gestionnaire)
            database.flush()  # Get ID without committing
...
async def update_gestionnaire(gestionnaire_id: int, gestionnaire_data: GestionnaireCreate, database: Session = Depends(get_db)) -> Gestionnaire:
    ...
    if gestionnaire_data.centredecongres_1 is not None:
        db_centredecongres_1 = database.query(CentredeCongres).filter(CentredeCongres.id == gestionnaire_data.centredecongres_1).first()
        if not db_centredecongres_1:
            raise HTTPException(status_code=400, detail="CentredeCongres not found")
        setattr(db_gestionnaire, 'centredecongres_1_id', gestionnaire_data.centredecongres_1)
    else:
        setattr(db_gestionnaire, 'centredecongres_1_id', None)
...
@app.post("/gestionnaire/methods/validerAnnulation/", response_model=None, tags=["Gestionnaire Methods"])
async def gestionnaire_validerAnnulation(
    params: dict = Body(default=None, embed=True),
    database: Session = Depends(get_db)
):
    """
    Execute the validerAnnulation class method on Gestionnaire.
...
@app.post("/gestionnaire/methods/gererIndisponibilite/", response_model=None, tags=["Gestionnaire Methods"])
async def gestionnaire_gererIndisponibilite(
    params: dict = Body(default=None, embed=True),
    database: Session = Depends(get_db)
):
    """
    Execute the gererIndisponibilite class method on Gestionnaire.
    This method operates on all Gestionnaire entities or performs class-level operations.

    Parameters (pass as JSON body):
    - element: Any    - dateDebut: Any    - dateFin: Any    - motif: Any    """
    try:
        # Capture stdout to include print outputs in the response
        import io
        import sys
