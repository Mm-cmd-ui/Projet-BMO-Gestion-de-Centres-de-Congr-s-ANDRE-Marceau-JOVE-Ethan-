...
inclut = Table(
    "inclut",
    Base.metadata,
    Column("reservation_3", ForeignKey("reservation.id"), primary_key=True),
    Column("materielsprestations", ForeignKey("materielsprestations.id"), primary_key=True),
)
tarifs_reservation = Table(
    "tarifs_reservation",
    Base.metadata,
    Column("tarifs_2", ForeignKey("tarifs.id"), primary_key=True),
    Column("reservation_4", ForeignKey("reservation.id"), primary_key=True),
)
applique_1 = Table(
    "applique_1",
    Base.metadata,
...
class Gestionnaire(Base):
    ...
    nom: Mapped[str] = mapped_column(String(100))
    centredecongres_1_id: Mapped[int] = mapped_column(ForeignKey("centredecongres.id"), nullable=True)
...
CentredeCongres.gestionnaire_1: Mapped[List["Gestionnaire"]] = relationship("Gestionnaire", back_populates="centredecongres_1", foreign_keys=[Gestionnaire.centredecongres_1_id])
...
Tarifs.reservation_4: Mapped[List["Reservation"]] = relationship("Reservation", secondary=tarifs_reservation, back_populates="tarifs_2")
...
Materielsprestations.reservation_3: Mapped[List["Reservation"]] = relationship("Reservation", secondary=inclut, back_populates="materielsprestations")
...
Reservation.materielsprestations: Mapped[List["Materielsprestations"]] = relationship("Materielsprestations", secondary=inclut, back_populates="reservation_3")
...
Reservation.tarifs_2: Mapped[List["Tarifs"]] = relationship("Tarifs", secondary=tarifs_reservation, back_populates="reservation_4")
...
Gestionnaire.centredecongres_1: Mapped["CentredeCongres"] = relationship("CentredeCongres", back_populates="gestionnaire_1", foreign_keys=[Gestionnaire.centredecongres_1_id])
